...
export class Migration20250917185544_updatedb6 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`DROP INDEX [Companies_slug_unique] ON [Companies];`);

    this.addSql(`alter table [Companies] alter column [slug] nvarchar(300) null;`);
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`alter table [Companies] alter column [slug] nvarchar(300) not null;`);
    this.addSql(`create unique index [Companies_slug_unique] on [Companies] (where ([slug] IS NOT NULL));`);
  }
