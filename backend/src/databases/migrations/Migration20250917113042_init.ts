...
export class Migration20250917113042_init extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`CREATE TABLE [BlogPosts] ([id] nvarchar(255) not null, [created_at] date not null, [updated_at] date null, [title] nvarchar(500) not null, [slug] nvarchar(500) not null, [content] text not null, [excerpt] nvarchar(1000) null, [cover_image_url] nvarchar(1000) null, [author_id] int not null, [is_published] bit not null CONSTRAINT [blogposts_is_published_default] DEFAULT 0, [published_at] date null, CONSTRAINT [BlogPosts_pkey] PRIMARY KEY ([id]));`);
    ...
    this.addSql(`CREATE TABLE [Skills] ([id] nvarchar(255) not null, [created_at] date not null, [updated_at] date null, [skill_id] nvarchar(255) not null, [name] nvarchar(200) not null, CONSTRAINT [Skills_pkey] PRIMARY KEY ([id]));`);
